...
\begin{tikzpicture}[scale=0.75, every node/.style={font=\small}]
  % Input
  \node[draw, minimum width=6cm, minimum height=1.2cm] (input) at (0,5) {Input Data};

  % Analyzer
  \node[draw, rounded corners, fill=gray!10, minimum width=4cm] (analyze) at (0,3.5) {Pattern Analyzer};

  % Platter 0
  \node[draw, rounded corners, fill=blue!10, minimum width=5cm, minimum height=2.8cm] (p0) at (-3.5,0) {};
  \node[font=\small\bfseries, anchor=north] at (-3.5, 1.2) {Platter 0 (Structured)};
  \node[font=\scriptsize, align=left, anchor=north] at (-3.5, 0.6) {Track 0: CONST(0xFF)\\Track 1: LINEAR(0, 1)\\Track 3: LINEAR\_WIDE(...)\\Meta: tracks 4--19 AllSame};

  % Platter 1
  \node[draw, rounded corners, fill=red!10, minimum width=5cm, minimum height=2.8cm] (p1) at (3.5,0) {};
  \node[font=\small\bfseries, anchor=north] at (3.5, 1.2) {Platter 1 (Raw)};
  \node[font=\scriptsize, align=left, anchor=north] at (3.5, 0.6) {Track 2: RAW (deflated)\\Track 20: RAW (deflated)\\Track 21: RAW (deflated)};

  % Arrows
  \draw[->, thick] (input) -- (analyze);
  \draw[->, thick] (analyze.south) -- ++(0,-0.5) -| (-3.5, 1.4) node[pos=0.25, left, font=\scriptsize] {patterns found};
  \draw[->, thick] (analyze.south) -- ++(0,-0.5) -| (3.5, 1.4) node[pos=0.25, right, font=\scriptsize] {no pattern};
\end{tikzpicture}
