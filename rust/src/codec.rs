...
/// Binary format:
///   Header (12 bytes):
///     Magic "PLTZ" (4), original_len u32 LE (4), sectors_per_track u16 LE (2), num_platters u16 LE (2)
///   Per platter:
///     num_tracks u16 LE (2)
///     Per track: track_idx u16 LE (2) + tag u8 (1) + payload
...
fn best_geometry(data: &[u8]) -> DiskGeometry {
    ...
    for &spt in SECTOR_CANDIDATES {
        ...
        let size: usize = encodings.iter()
            .map(|e| estimate_track_size(e, spt))
            .sum();
        if size < best_size {
            best_size = size;
            best_geom = geom;
...
fn split_platters(tracks: &[Vec<u8>]) -> Vec<Vec<TrackEncoding>> {
    ...
    if !structured.is_empty() {
        platters.push(structured);
    }
    if !raw.is_empty() {
        platters.push(raw);
    }
    if platters.is_empty() {
        platters.push(Vec::new());
    }
    platters
}
...
fn encode_track(buf: &mut Vec<u8>, enc: &TrackEncoding, sectors: usize, raw_comp: RawCompressor) {
    match &enc.params {
...
fn decode_track(buf: &[u8], offset: usize, sectors: usize) -> Result<(u16, Vec<u8>, usize), String> {
    ...
    let track = match tag {
        ...
        PatternType::Linear => {
            ...
            pos += 4;
            (0..sectors).map(|i| (start + i as i16 * step) as u8).collect()
        }
        PatternType::Rle => {
            ...
            for _ in 0..num_runs {
                ...
                let count = buf[pos + 1];
                pos += 2;
                track.extend(std::iter::repeat(val).take(count as usize));
            }
        ...
        PatternType::Periodic => {
            // Read components but skip reconstruction (no FFT in pure Rust without deps)
            ...
            // Reconstruct using DFT synthesis
            let mut components: Vec<(usize, f64, f64)> = Vec::with_capacity(num_comps);
        ...
        PatternType::RawCompressed => {
            let _method = buf[pos];
            let comp_len = u16::from_le_bytes(buf[pos + 1..pos + 3].try_into().unwrap()) as usize;
            pos += 3;
...
/// Inverse real FFT (manual implementation for decompression).
fn irfft(real: &[f64], imag: &[f64], n: usize) -> Vec<u8> {
    ...
    for t in 0..n {
        ...
        result.push((sum / n as f64 * 2.0).round() as u8);
    }
    // Fix DC component (index 0 is not doubled) and Nyquist if n is even
    // Actually, proper irfft: X[0] contributes once, X[n/2] contributes once, rest twice
    let mut result2 = Vec::with_capacity(n);
    for t in 0..n {
        let mut sum = real[0]; // DC (no doubling)
        for k in 1..real.len() - 1 {
            let angle = 2.0 * std::f64::consts::PI * (k as f64) * (t as f64) / (n as f64);
            sum += 2.0 * (real[k] * angle.cos() - imag[k] * angle.sin());
        }
        // Nyquist (last component, no doubling if n is even)
        if n % 2 == 0 && real.len() > 1 {
            let k = real.len() - 1;
            let angle = 2.0 * std::f64::consts::PI * (k as f64) * (t as f64) / (n as f64);
            sum += real[k] * angle.cos() - imag[k] * angle.sin();
        }
        result2.push((sum / n as f64).round() as u8);
    }
    result2
}
...
mod tests {
    ...
    #[test]
    fn test_empty() { roundtrip(&[]); }

    #[test]
    fn test_const() { roundtrip(&[0xFFu8; 1024]); }
    ...
    #[test]
    fn test_repeat() {
        let data: Vec<u8> = [0xAB, 0xCD].iter().copied().cycle().take(1024).collect();
        roundtrip(&data);
    }
    ...
    fn test_linear_wide_u32() {
        // Simulate a vector table
        let mut data = Vec::new();
        for i in 0u32..64 {
            data.extend_from_slice(&(0x80000000u32 + i * 0x20).to_le_bytes());
        }
        roundtrip(&data);
    }
    ...
    #[test]
    fn test_delta() {
        // Slowly varying data with few unique deltas
        let data: Vec<u8> = (0..512).map(|i| (128u8).wrapping_add((i % 5) as u8).wrapping_sub(2)).collect();
        roundtrip(&data);
    }

    #[test]
    fn test_single_byte() { roundtrip(&[0x42]); }

    #[test]
    fn test_small_string() { roundtrip(b"Hello, Platter!"); }
}
